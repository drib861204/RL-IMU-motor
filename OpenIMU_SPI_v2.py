...
class SpiOpenIMU:
    def __init__(self, target_module = "300ZI", fw='0.0', cs_pin = 19, interrupt_pin = 20, drdy_status = True):
        '''
    ...
    def single_read(self, target_register):
        ...
        if True:
            ...
            #resp_single = self.spi.xfer2([target_register,0x00,0x00,0x00],0,10)         
            resp_single.append(self.spi.xfer2([target_register, 0x00],0,10))
            resp_single.append(self.spi.xfer2([0x00, 0x00],0,10))
            GPIO.output(self.cs_channel,GPIO.HIGH)    

            #rate.append(self.combine_reg(resp_single[2],resp_single[3])*100000)             
            return self.combine_reg(resp_single[1][0],resp_single[1][1])            
    ...
    def burst_read(self, first_register, subregister_num):   
        ...
        if True:     #300,330 is here                   
            ...
            resp.append(self.spi.xfer2([first_register],0,10)[0])
            resp.append(self.spi.xfer2([0x00],0,10)[0])
            for i in range(2*subregister_num):
                resp.append(self.spi.xfer2([0x00],0,10)[0])

            #resp = self.spi.xfer2(first_register_send,0,10)
            GPIO.output(self.cs_channel,GPIO.HIGH)
            ...
            #unit:deg
            if ('300' in self.module) and first_register == 0x3D:
